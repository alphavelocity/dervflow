...
class TestArbitrageFreeConditions:
    ...
    def test_american_option_value_bounds(self):
        ...
        for spot, strike, rate, dividend, vol, time, opt_type in test_cases:
            european_price = tree.price(spot, strike, rate, dividend, vol, time, 
                                       100, 'european', opt_type, 'crr')
            american_price = tree.price(spot, strike, rate, dividend, vol, time, 
                                       100, 'american', opt_type, 'crr')
...
class TestImpliedVolatilityProperties:
    ...
    def test_iv_monotonic_in_price(self):
        ...
        # Calculate IVs
        ivs = [bs.implied_vol(price, spot, strike, rate, dividend, time, 'call') 
               for price in prices]
