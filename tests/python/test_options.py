    ...
    def test_batch_vs_single(self):
        ...
        for i in range(len(spots)):
            single_price = self.bs.price(
                spots[i], strikes[i], rates[i], dividends[i],
...
class TestImpliedVolatility:
    ...
    def test_iv_batch_vs_single(self):
        ...
        market_prices = np.array([
            self.bs.price(spots[i], strikes[i], rates[i], dividends[i], 
                         true_vols[i], times[i], option_types[i])
            for i in range(len(spots))
        ])
        ...
        for i in range(len(spots)):
            single_iv = self.bs.implied_vol(
                market_prices[i], spots[i], strikes[i], rates[i], 
                dividends[i], times[i], option_types[i]
            )
    ...
    def test_iv_batch_mixed_types(self):
        ...
        market_prices = np.array([
            self.bs.price(spots[i], strikes[i], rates[i], dividends[i], 
                         true_vol, times[i], option_types[i])
            for i in range(n)
        ])
...
class TestBinomialTreePricing:
    ...
    def test_convergence_to_black_scholes_50_steps(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call')
        tree_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 50, 'european', 'call', 'crr')
    ...
    def test_convergence_to_black_scholes_100_steps(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call')
        tree_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'call', 'crr')
    ...
    def test_convergence_to_black_scholes_200_steps(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call')
        tree_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 200, 'european', 'call', 'crr')
    ...
    def test_convergence_to_black_scholes_500_steps(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call')
        tree_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 500, 'european', 'call', 'crr')
    ...
    def test_american_call_no_dividend(self):
        ...
        european_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'call', 'crr')
        american_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'american', 'call', 'crr')
    ...
    def test_american_put_early_exercise(self):
        ...
        european_price = self.tree.price(100.0, 120.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'put', 'crr')
        american_price = self.tree.price(100.0, 120.0, 0.05, 0.0, 0.2, 1.0, 100, 'american', 'put', 'crr')
    ...
    def test_american_put_greater_than_european(self):
        ...
        for strike in [90.0, 100.0, 110.0, 120.0]:
            european_price = self.tree.price(100.0, strike, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'put', 'crr')
            american_price = self.tree.price(100.0, strike, 0.05, 0.0, 0.2, 1.0, 100, 'american', 'put', 'crr')
    ...
    def test_american_call_with_dividend(self):
        ...
        european_price = self.tree.price(110.0, 100.0, 0.05, 0.05, 0.2, 1.0, 100, 'european', 'call', 'crr')
        american_price = self.tree.price(110.0, 100.0, 0.05, 0.05, 0.2, 1.0, 100, 'american', 'call', 'crr')
    ...
    def test_crr_vs_jr_european_call(self):
        ...
        crr_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'call', 'crr')
    ...
    def test_crr_vs_jr_european_put(self):
        ...
        crr_price = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'put', 'crr')
    ...
    def test_crr_vs_jr_american_put(self):
        ...
        crr_price = self.tree.price(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 100, 'american', 'put', 'crr')
    ...
    def test_call_with_dividend(self):
        """Test that dividend reduces call price"""
        price_with_div = self.tree.price(100.0, 100.0, 0.05, 0.02, 0.2, 1.0, 100, 'european', 'call', 'crr')
        price_no_div = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'call', 'crr')
        assert price_with_div < price_no_div

    def test_put_with_dividend(self):
        """Test that dividend increases put price"""
        price_with_div = self.tree.price(100.0, 100.0, 0.05, 0.02, 0.2, 1.0, 100, 'european', 'put', 'crr')
        price_no_div = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'put', 'crr')
        assert price_with_div > price_no_div
    ...
    def test_default_tree_type(self):
        ...
        price_crr = self.tree.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 100, 'european', 'call', 'crr')
    ...
    def test_batch_vs_single_european(self):
        ...
        for i in range(len(spots)):
            single_price = self.tree.price(
                spots[i], strikes[i], rates[i], dividends[i],
                volatilities[i], times[i], 100, styles[i], option_types[i], 'crr'
            )
            assert abs(batch_prices[i] - single_price) < 1e-10
...
class TestMonteCarloEuropeanPricing:
    ...
    def test_european_call_convergence(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call')
        result = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
    ...
    def test_european_put_convergence(self):
        ...
        bs_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put')
        result = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
    ...
    def test_itm_call_convergence(self):
        ...
        result = self.mc.price_european(110.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
    ...
    def test_otm_put_convergence(self):
        ...
        result = self.mc.price_european(110.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
    ...
    def test_antithetic_variance_reduction(self):
        ...
        result_no_av = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                              num_paths=10000, use_antithetic=False, seed=42)
        # With antithetic variates
        result_with_av = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
    ...
    def test_reproducibility_with_seed(self):
        ...
        result1 = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                         num_paths=10000, seed=42)
        result2 = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                         num_paths=10000, seed=42)
    ...
    def test_parallel_vs_sequential(self):
        ...
        result_parallel = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                                 num_paths=10000, seed=42, parallel=True)
        result_sequential = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                                   num_paths=10000, seed=42, parallel=False)
    ...
    def test_zero_time_equals_intrinsic(self):
        ...
        result = self.mc.price_european(110.0, 100.0, 0.05, 0.0, 0.2, 0.0, 'call',
                                        num_paths=10000, seed=42)
        intrinsic = 10.0
    ...
    def test_with_dividend(self):
        ...
        result = self.mc.price_european(100.0, 100.0, 0.05, 0.02, 0.2, 1.0, 'call',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
    ...
    def test_high_volatility(self):
        ...
        result = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.5, 1.0, 'call',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
    ...
    def test_low_volatility(self):
        ...
        result = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.05, 1.0, 'call',
                                        num_paths=50000, use_antithetic=True, seed=42)
        mc_price = result['price']
        std_error = result['std_error']
...
class TestMonteCarloAmericanPricing:
    ...
    def test_american_put_early_exercise_premium(self):
        ...
        # European put price
        euro_result = self.mc.price_european(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                             num_paths=10000, seed=42)
        # American put price
        amer_result = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
    ...
    def test_american_call_no_dividend_equals_european(self):
        """Test that American call equals European call with no dividend"""
        # Without dividends, American call should not be exercised early
        euro_result = self.mc.price_european(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                             num_paths=10000, seed=42)
        amer_result = self.mc.price_american(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                                             num_paths=10000, num_steps=50, seed=42)
    ...
    def test_american_put_vs_tree(self):
        ...
        tree_price = self.tree.price(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 100, 'american', 'put', 'crr')
        mc_result = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                           num_paths=20000, num_steps=50, seed=42)
    ...
    def test_deep_itm_put_early_exercise(self):
        ...
        euro_result = self.mc.price_european(80.0, 120.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                             num_paths=10000, seed=42)
        amer_result = self.mc.price_american(80.0, 120.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                             num_paths=10000, num_steps=50, seed=42)
    ...
    def test_reproducibility_with_seed(self):
        """Test that results are reproducible with same seed"""
        result1 = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                         num_paths=10000, num_steps=50, seed=42)
        result2 = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                         num_paths=10000, num_steps=50, seed=42)
    ...
    def test_parallel_vs_sequential(self):
        """Test that parallel and sequential execution produce similar results"""
        result_parallel = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                                 num_paths=5000, num_steps=50, seed=42, parallel=True)
        result_sequential = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                                   num_paths=5000, num_steps=50, seed=42, parallel=False)
    ...
    def test_more_steps_better_accuracy(self):
        ...
        # Fewer steps
        result_few = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                            num_paths=10000, num_steps=20, seed=42)
        # More steps
        result_many = self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                             num_paths=10000, num_steps=100, seed=42)
    ...
    def test_zero_time_equals_intrinsic(self):
        """Test that option at expiry equals intrinsic value"""
        result = self.mc.price_american(90.0, 100.0, 0.05, 0.0, 0.2, 0.0, 'put',
                                        num_paths=10000, num_steps=50, seed=42)
        intrinsic = 10.0
        assert abs(result['price'] - intrinsic) < 1e-10
        assert result['std_error'] == 0.0
    ...
    def test_with_dividend(self):
        ...
        result = self.mc.price_american(100.0, 110.0, 0.05, 0.02, 0.2, 1.0, 'put',
                                        num_paths=10000, num_steps=50, seed=42)
    ...
    def test_invalid_num_paths(self):
        """Test error handling for zero paths"""
        with pytest.raises(ValueError, match="Number of paths must be positive"):
            self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                  num_paths=0, num_steps=50)

    def test_invalid_num_steps(self):
        """Test error handling for zero steps"""
        with pytest.raises(ValueError, match="Number of steps must be positive"):
            self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'put',
                                  num_paths=10000, num_steps=0)
    ...
    def test_invalid_option_type(self):
        """Test error handling for invalid option type"""
        with pytest.raises(ValueError, match="Invalid option type"):
            self.mc.price_american(100.0, 110.0, 0.05, 0.0, 0.2, 1.0, 'invalid',
                                  num_paths=10000, num_steps=50)
...
class TestAsianOptions:
    ...
    def test_arithmetic_asian_call_fixed_strike(self):
        ...
        price = self.asian.price_arithmetic(
            ...
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
    ...
    def test_arithmetic_asian_call_floating_strike(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=12, fixed_strike=False, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_geometric_cheaper_than_arithmetic(self):
        ...
        arith_price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=20000, seed=42
        )
    ...
    def test_more_observations_cheaper(self):
        ...
        price_few = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=4, fixed_strike=True, num_paths=10000, seed=42
        )
        price_many = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=52, fixed_strike=True, num_paths=10000, seed=42
        )
    ...
    def test_invalid_num_observations(self):
        ...
        with pytest.raises(ValueError, match="Number of observations must be positive"):
            self.asian.price_arithmetic(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                num_observations=0, fixed_strike=True, num_paths=10000
            )
...
class TestBarrierOptions:
    ...
    def test_up_and_out_call(self):
        ...
        price = self.barrier.price(
            100.0, 95.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=120.0, barrier_type='up-and-out', rebate=0.0
        )
    ...
    def test_down_and_out_put(self):
        ...
        price = self.barrier.price(
            100.0, 105.0, 0.05, 0.0, 0.2, 1.0, 'put',
            barrier=90.0, barrier_type='down-and-out', rebate=0.0
        )
        # Barrier option should be non-negative and cheaper than vanilla
    ...
    def test_up_and_out_put(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
            barrier=110.0, barrier_type='up-and-out', rebate=0.0
        )
        # Barrier option should be cheaper than vanilla
        vanilla_price = self.bs.price(100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put')
        assert 0.0 < price < vanilla_price
    ...
    def test_down_and_in_call(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=90.0, barrier_type='down-and-in', rebate=0.0
        )
        assert price > 0.0
    ...
    def test_up_and_in_call(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=120.0, barrier_type='up-and-in', rebate=0.0
        )
        assert price > 0.0
    ...
    def test_barrier_with_rebate(self):
        ...
        price_with_rebate = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=90.0, barrier_type='down-and-out', rebate=5.0
        )
        # Rebate adds value
    ...
    def test_barrier_closer_to_spot_cheaper(self):
        ...
        price_far = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=80.0, barrier_type='down-and-out', rebate=0.0
        )
        price_close = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=95.0, barrier_type='down-and-out', rebate=0.0
        )
    ...
    def test_invalid_barrier_up_below_spot(self):
        ...
        with pytest.raises(ValueError, match="Up barrier must be above spot"):
            self.barrier.price(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                barrier=90.0, barrier_type='up-and-out', rebate=0.0
            )
    ...
    def test_invalid_barrier_down_above_spot(self):
        ...
        with pytest.raises(ValueError, match="Down barrier must be below spot"):
            self.barrier.price(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                barrier=110.0, barrier_type='down-and-out', rebate=0.0
            )
...
class TestLookbackOptions:
    ...
    def test_fixed_strike_lookback_call(self):
        ...
        price = self.lookback.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            lookback_type='fixed', current_extremum=None
        )
    ...
    def test_floating_strike_lookback_put(self):
        ...
        price = self.lookback.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
            lookback_type='floating', current_extremum=105.0
        )
        # Floating strike lookback should have positive value
        assert price > 0.0
    ...
    def test_higher_volatility_increases_value(self):
        ...
        price_low_vol = self.lookback.price(
            100.0, 100.0, 0.05, 0.0, 0.15, 1.0, 'call',
            lookback_type='fixed', current_extremum=None
        )
        price_high_vol = self.lookback.price(
            100.0, 100.0, 0.05, 0.0, 0.30, 1.0, 'call',
            lookback_type='fixed', current_extremum=None
        )
...
class TestDigitalOptions:
    ...
    def test_asset_or_nothing_call(self):
        ...
        price = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='asset-or-nothing', cash_payout=1.0
        )
    ...
    def test_digital_call_plus_put_equals_discounted_payout(self):
        ...
        call_price = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=payout
        )
    ...
    def test_itm_digital_call_higher_value(self):
        ...
        itm_price = self.digital.price(
            110.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=10.0
        )
    ...
    def test_otm_digital_put_lower_value(self):
        ...
        otm_price = self.digital.price(
            110.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
            digital_type='cash-or-nothing', cash_payout=10.0
        )
    ...
    def test_zero_time_itm_equals_payout(self):
        ...
        price = self.digital.price(
            110.0, 100.0, 0.05, 0.0, 0.2, 0.0, 'call',
            digital_type='cash-or-nothing', cash_payout=10.0
        )
    ...
    def test_zero_time_otm_equals_zero(self):
        ...
        price = self.digital.price(
            90.0, 100.0, 0.05, 0.0, 0.2, 0.0, 'call',
            digital_type='cash-or-nothing', cash_payout=10.0
        )
    ...
    def test_higher_payout_increases_value(self):
        ...
        price_low = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=5.0
        )
        price_high = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=15.0
        )
...
class TestAsianOptionsEdgeCases:
    ...
    def test_very_high_volatility(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 1.0, 1.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_very_low_volatility(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.01, 1.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_deep_itm_call(self):
        ...
        price = self.asian.price_arithmetic(
            150.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
    ...
    def test_deep_otm_put(self):
        ...
        price = self.asian.price_arithmetic(
            150.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'put',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price >= 0.0
    ...
    def test_single_observation(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=1, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_many_observations(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            num_observations=252, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_short_maturity(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 0.01, 'call',
            num_observations=2, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price >= 0.0
    ...
    def test_long_maturity(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.0, 0.2, 5.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
    ...
    def test_high_dividend_yield(self):
        ...
        price = self.asian.price_arithmetic(
            100.0, 100.0, 0.05, 0.10, 0.2, 1.0, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        assert price > 0.0
        ...
        for strike in [90.0, 100.0, 110.0]:
            for vol in [0.1, 0.2, 0.3]:
                geom = self.asian.price_geometric(
                    100.0, strike, 0.05, 0.0, vol, 1.0, 'call',
                    num_observations=12, fixed_strike=True
                )
...
class TestBarrierOptionsEdgeCases:
    ...
    def test_barrier_very_close_to_spot(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=100.1, barrier_type='up-and-out', rebate=0.0
        )
        assert price >= 0.0
    ...
    def test_barrier_very_far_from_spot(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=200.0, barrier_type='up-and-out', rebate=0.0
        )
    ...
    def test_high_rebate(self):
        ...
        price = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            barrier=90.0, barrier_type='down-and-out', rebate=50.0
        )
        assert price > 0.0
    ...
    def test_all_barrier_types_positive(self):
        ...
        for btype in barrier_types:
            ...
            price = self.barrier.price(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                barrier=barrier, barrier_type=btype, rebate=0.0
            )
    ...
    def test_barrier_put_call_symmetry(self):
        ...
        for opt_type in ['call', 'put']:
            price = self.barrier.price(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, opt_type,
                barrier=90.0, barrier_type='down-and-out', rebate=0.0
            )
            assert price >= 0.0
    ...
    def test_extreme_volatility_barrier(self):
        ...
        price_high = self.barrier.price(
            100.0, 100.0, 0.05, 0.0, 0.8, 1.0, 'call',
            barrier=90.0, barrier_type='down-and-out', rebate=0.0
        )
...
class TestLookbackOptionsEdgeCases:
    ...
    def test_extreme_volatility_lookback(self):
        ...
        price_low = self.lookback.price(
            100.0, 100.0, 0.05, 0.0, 0.05, 1.0, 'call',
            lookback_type='fixed', current_extremum=None
        )
    ...
    def test_lookback_all_types(self):
        ...
        for lookback_type in ['fixed', 'floating']:
            for option_type in ['call', 'put']:
                price = self.lookback.price(
                    100.0, 100.0, 0.05, 0.0, 0.2, 1.0, option_type,
                    lookback_type=lookback_type, current_extremum=None
                )
    ...
    def test_floating_with_various_extrema(self):
        ...
        for ext in extrema:
            price = self.lookback.price(
                100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
                lookback_type='floating', current_extremum=ext
            )
            prices.append(price)
            assert price > 0.0
...
class TestDigitalOptionsEdgeCases:
    ...
    def test_extreme_payouts(self):
        ...
        price_small = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=0.01
        )
        price_large = self.digital.price(
            100.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=1000.0
        )
    ...
    def test_deep_otm_digital(self):
        ...
        price = self.digital.price(
            50.0, 100.0, 0.05, 0.0, 0.2, 1.0, 'call',
            digital_type='cash-or-nothing', cash_payout=10.0
        )
    ...
    def test_various_strikes_digital(self):
        ...
        for strike in strikes:
            price = self.digital.price(
                100.0, strike, 0.05, 0.0, 0.2, 1.0, 'call',
                digital_type='cash-or-nothing', cash_payout=10.0
            )
            prices.append(price)
            assert price >= 0.0
...
class TestExoticOptionsIntegration:
    ...
    def setup_method(self):
        """Set up test fixtures"""
        from dervflow import AsianOption, BarrierOption, LookbackOption, DigitalOption, BlackScholesModel
        self.asian = AsianOption()
        self.barrier = BarrierOption()
        self.lookback = LookbackOption()
        self.digital = DigitalOption()
    ...
    def test_all_exotic_types_produce_valid_prices(self):
        ...
        asian_price = self.asian.price_arithmetic(
            spot, strike, rate, div, vol, time, 'call',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
    ...
    def test_consistency_across_option_types(self):
        ...
        asian_put = self.asian.price_arithmetic(
            spot, strike, rate, div, vol, time, 'put',
            num_observations=12, fixed_strike=True, num_paths=10000, seed=42
        )
        ...
        # All should be positive
        assert all(p > 0.0 for p in [asian_call, asian_put, barrier_call, barrier_put,
                                      lookback_call, lookback_put, digital_call, digital_put])
