...
class TestRequirement2RiskCalculation:
    ...
    def test_2_5_value_at_risk(self):
        ...
        # Monte Carlo VaR
        var_mc = risk.var(confidence_level=0.95, method='monte_carlo', mean=0.001, std_dev=0.02, num_simulations=10000)
...
class TestRequirement3VolatilityAnalysis:
    ...
    def test_3_5_sabr_calibration(self):
        ...
        market_vols = [
            true_model.implied_volatility(forward, float(strike), maturity)
...
class TestRequirement4PortfolioOptimization:
    ...
    def test_4_2_constraint_handling(self):
        ...
        n = 5
        result = optimizer.optimize(
            min_weights=np.full(n, 0.0),
            max_weights=np.full(n, 0.3)
        )
    ...
    def test_4_5_infeasible_optimization_error(self):
        ...
        assert 'infeasible' in str(exc_info.value).lower() or 'constraint' in str(exc_info.value).lower()
...
class TestRequirement8PythonAPI:
    ...
    def test_8_1_expose_all_functionality(self):
        ...
        from dervflow import options, risk, portfolio, yield_curve, timeseries, monte_carlo
