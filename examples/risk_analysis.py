...
from dervflow import GreeksCalculator, RiskMetrics, BlackScholesModel
...
def greeks_sensitivity_example():
    ...
    for s in spot_range:
        greeks = greeks_calc.calculate(
            s, strike, rate, dividend, volatility, time,
            option_type='call'
        )
    ...
    for vol in vol_range:
        greeks = greeks_calc.calculate(
            spot, strike, rate, dividend, vol, time,
            option_type='call'
        )
...
def var_calculation_example():
    ...
    for conf in confidence_levels:
        ...
        print(f"{conf:.0%}{'':>12} {var_value:>13.4%}  "
    ...
    for conf in confidence_levels:
        ...
        print(f"{conf:.0%}{'':>12} {cvar_value:>13.4%}  "
              f"Expected loss when VaR is exceeded")
...
def risk_adjusted_metrics_example():
    ...
    for name, (mean_ret, vol) in strategies.items():
        ...
        print(f"{name:<15} {annual_return:>8.2%} {annual_vol:>8.2%} "
